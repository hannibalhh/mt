...
static const char * c2_debug_family_names[14] = { "g", "FG", "FF", "FD",
...
static void sf_gateway_c2_tisch(SFc2_tischInstanceStruct *chartInstance)
{
  ...
  c2_e_hoistedGlobal = *chartInstance->c2_x;
  c2_f_hoistedGlobal = *chartInstance->c2_v1;
  c2_g_hoistedGlobal = *chartInstance->c2_v2;
  c2_b_u = c2_hoistedGlobal;
  c2_b_m = c2_b_hoistedGlobal;
  c2_b_c = c2_c_hoistedGlobal;
  c2_b_d = c2_d_hoistedGlobal;
  c2_b_x = c2_e_hoistedGlobal;
  c2_b_v1 = c2_f_hoistedGlobal;
  c2_b_v2 = c2_g_hoistedGlobal;
  ...
  _SFD_SYMBOL_SCOPE_ADD_EML(&c2_b_c, 8U, c2_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c2_b_d, 9U, c2_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c2_b_x, 10U, c2_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c2_b_v1, 11U, c2_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c2_b_v2, 12U, c2_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c2_b_xpp, 13U, c2_sf_marshallOut,
    c2_sf_marshallIn);
  CV_EML_FCN(0, 0);
  ...
  *chartInstance->c2_xpp = c2_b_xpp;
  ...
  _SFD_DATA_RANGE_CHECK(*chartInstance->c2_c, 3U);
  _SFD_DATA_RANGE_CHECK(*chartInstance->c2_d, 4U);
  _SFD_DATA_RANGE_CHECK(*chartInstance->c2_x, 5U);
  _SFD_DATA_RANGE_CHECK(*chartInstance->c2_v1, 6U);
  _SFD_DATA_RANGE_CHECK(*chartInstance->c2_v2, 7U);
}
  ...
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        _SFD_SET_DATA_VALUE_PTR(3U, chartInstance->c2_c);
        _SFD_SET_DATA_VALUE_PTR(4U, chartInstance->c2_d);
        _SFD_SET_DATA_VALUE_PTR(5U, chartInstance->c2_x);
        _SFD_SET_DATA_VALUE_PTR(6U, chartInstance->c2_v1);
        _SFD_SET_DATA_VALUE_PTR(7U, chartInstance->c2_v2);
      }
    } else {
